...
namespace Disney.iDash.SRR.BusinessLayer
{
    public class StockInfo : BusinessBase
    {
        ...
        private string _guidLock = System.Guid.NewGuid().ToString();
        ...
        private DataTable GetLocks(decimal deptId, string userName)
        {
           string result = "";
           ...
           try
           {
               ...
               {
                   var sql = Properties.Resources.SQLStorItemAssignmentGetLocks;
                   sql = sql.Replace("<dept>", deptId.ToString());
                   sql = sql.Replace("<userName>", userName);

                   dataTbl = base.Factory.CreateTable(sql);                    
               }
           }
           catch (Exception ex)
           {
        ...
        public bool Lock(decimal deptId, string userName, out string actionResult)
        {
           bool result = false;
           actionResult = "";
            ...
            {       
               ...
               if (dataTbl != null)
               {
                   ...
                   {
                       string usr = dataTbl.Rows[0]["UserName"].ToString();
                       decimal dptId = decimal.Parse(dataTbl.Rows[0]["DeptId"].ToString());
                   ...
                   else
                   {
                       if (base.Factory.OpenConnection())
                       {
                           var cmdInsert = Factory.CreateCommand(Properties.Resources.SQLStorItemAssignmentLock.Replace("<dept>", deptId.ToString()).Replace("<username>", userName).Replace("<guid>", _guidLock));
        ...
        public bool ReleaseLock(decimal deptId, string userName)
        {
           try
           {
               if (base.Factory.OpenConnection())
               {
                   var cmdDelete = Factory.CreateCommand(Properties.Resources.SQLStorItemAssignmentDeleteLock.Replace("<dept>", deptId.ToString()).Replace("<username>", userName).Replace("<guid>",_guidLock));

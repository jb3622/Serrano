...
//                            ReleaseLock(row[colWorkBench].ToString(), new StockItem(row));
        ...
        public void RefreshFocusGroupsCoverStores(string workbench)
        {
            try
            {
			    if (Factory.OpenConnection())
				{
					var cmd = Factory.CreateCommand("DS889FS1", CommandType.StoredProcedure,
						Factory.CreateParameter("P_DEPT", this.DepartmentId, OleDbType.Decimal, 3, ParameterDirection.Input),
						Factory.CreateParameter("P_PERIOD", workbench, OleDbType.Char, 1, ParameterDirection.Input),
						Factory.CreateParameter("P_ERROR", string.Empty, OleDbType.Char, 128, ParameterDirection.InputOutput));
					cmd.ExecuteNonQuery();				
				}
            }
			catch (Exception ex)
			{
                ExceptionHandler.RaiseException(ex, "Could not run program DS889FS1.");
			}
			finally
			{

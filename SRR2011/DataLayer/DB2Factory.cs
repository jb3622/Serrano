            ...
            if (_con.State == ConnectionState.Closed)
            {
                ...
                catch (OleDbException ex)
                {
                    if (ex.ErrorCode == -2147467259)
                    {
                       ...
                       errMessage += "SRR will now terminate.";
                       Exception newEx = new Exception(errMessage);
                       
                       ExceptionHandler exHandler = new Shared.ExceptionHandler();
                       exHandler.RaiseException(newEx, "Server Connection Error", true);
                       Environment.Exit(-1);
                    }                    
                    ExceptionHandler.RaiseException(ex, "Server Connection Error");
                    result = false;
                }
                catch (Exception ex)
                {

...
namespace Disney.iDash.DataLayer
{
    public sealed class DB2Factory
    {
        ...
        private volatile OleDbConnection _con = new OleDbConnection();
        ...
        private static Dictionary<int, bool> _connectionPool = new Dictionary<int, bool>();
		...
		public DB2Factory()
		{
            ...
            _con.StateChange += ((sender, e) =>
            {
                if (_connectionPool.ContainsKey(_con.GetHashCode()))
                    _connectionPool[_con.GetHashCode()] = _con.State == ConnectionState.Open;
                else
                    _connectionPool.Add(_con.GetHashCode(), _con.State == ConnectionState.Open);

                if (e.CurrentState == ConnectionState.Open)
                    ConnectionCount++;

                if (System.Diagnostics.Debugger.IsAttached)
                    Console.WriteLine("Connection state {0} at {1} OpenCount {2} ClosedCount {3}", e.CurrentState, DateTime.Now, _connectionPool.Count((kvp) => kvp.Value == true), _connectionPool.Count((kvp) => kvp.Value == false));
        ...
        public OleDbTransaction BeginTransaction()
        {
            ...
            /*
             * The Ole Provider does not support nested transactions which
             * can be created in a background thread.  Hence we have three options
             * 
             * Use a different provider (not possible at present)
             * Use multiple connections (not good from a resource management viewpoint)
            ...
            */
			...
			if (_con.State == ConnectionState.Open)
			{
                ...
                {
                    Console.WriteLine("OleDbTransaction {0}", ex.Message);
                }
			}
        ...
        public bool OpenConnection(string connectionString)
        {
            ...
            if (_con.State != ConnectionState.Closed && _con.ConnectionString != connectionString)
            {
                _con.Close();
                ConnectionCount = 0;
            }
